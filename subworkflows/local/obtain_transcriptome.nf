...
workflow OBTAIN_TRANSCRIPTOME {
    ...
    //
    // download if not given
    //
	if(!fasta) {
		println "download fasta"
    ...
    if (fasta.endsWith('.gz')) {
        GUNZIP_FASTA (
            Channel.fromPath(fasta).map{ it -> [[id:it[0].baseName], it] }
        )
    ...
    if (gtf.endsWith('.gz')) {
        GUNZIP_GTF (
            Channel.fromPath(gtf).map{ it -> [[id:it[0].baseName], it] }
        )
        ch_gtf = GUNZIP_GTF.out.gunzip.map{ meta, gtf -> [gtf] }.collect()
        ch_versions = ch_versions.mix(GUNZIP_GTF.out.versions)
    } else {
        ch_gtf = Channel.fromPath(gtf).collect()
    }
